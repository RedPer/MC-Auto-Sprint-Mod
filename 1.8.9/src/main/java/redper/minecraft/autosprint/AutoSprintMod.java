...
public class AutoSprintMod {
    ...
    private void syncConfig(boolean load) {
        ...
        prop = config.get(Configuration.CATEGORY_CLIENT, "isSprintToggled", false, "Determines if sprint is toggled.");
        prop.setLanguageKey("config.client.sprinttoggled");
        prop.setRequiresMcRestart(true);
        prop.setRequiresWorldRestart(false);
        prop.setShowInGui(false);

        prop = config.get(Configuration.CATEGORY_CLIENT, "doRememberSprint", true, "Do Remember Sprint State after Minecraft exit.");
        prop.setLanguageKey("config.client.remembersprint");
        prop.setRequiresMcRestart(false);
        prop.setRequiresWorldRestart(false);
        prop.setShowInGui(true);

        prop = config.get(Configuration.CATEGORY_CLIENT, "hexTextColor", "aaffaa", "The Hex Color Of All The text displayed");
        prop.setLanguageKey("config.client.textcolor");
        prop.setRequiresMcRestart(false);
        prop.setRequiresWorldRestart(false);
        prop.setShowInGui(true);

        prop = config.get(Configuration.CATEGORY_CLIENT, "useTextBgr", true, "Determines if some background is shown under the text");
        prop.setLanguageKey("config.client.textbackground");
        prop.setRequiresMcRestart(false);
        prop.setRequiresWorldRestart(false);
        prop.setShowInGui(true);

        prop = config.get(Configuration.CATEGORY_CLIENT, "backgroundColor", "172331", "Color of the background under the text");
